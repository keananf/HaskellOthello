...
gameLoop idToPlay chanInput chanOutputP1 chanOutputP2 = do
  ...
  (playerID, message2) <- waitForCorrectInputChan idToPlay chanInput

  --DO SOME COMPUTATION ON USER INPUT

  writeChan chanOutputP1 (playerID, message)
  writeChan chanOutputP2 (playerID, message)
  writeChan chanOutputP1 (playerID, message2)
  writeChan chanOutputP2 (playerID, message2)

  if idToPlay == 1
    then gameLoop 2 chanInput chanOutputP1 chanOutputP2
    else gameLoop 1 chanInput chanOutputP1 chanOutputP2
