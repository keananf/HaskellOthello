...
handleInput (EventKey (MouseButton LeftButton) Up m (x, y)) world
  | not (gameOver board)&& hasNetwork && inRange board (x', y') = unsafePerformIO (networkNewWorld world x' y')
...
networkNewWorld :: World -> Int -> Int -> IO World
networkNewWorld world x y = case makeMove board col (x,y) of
    (Just newBoard') -> do let newWorld = world {gameboard = newBoard', turn = newCol, oldworld = world}
                           sendAcrossNetwork hasHandle x y
                           return newWorld
    (Nothing) -> do let newWorld = world
                    print ""
                    return newWorld
    where board = gameboard world
          col = turn world
